<template>
  <a-modal
    title="修改菜单"
    :width="640"
    :visible="visible"
    :confirmLoading="loading"
    @ok="handleOk"
    @cancel="handleCancel"
  >
    <a-spin :spinning="loading">
      <a-form :form="form" @submit="handleSubmit" :labelCol="labelCol" :wrapperCol="wrapperCol">
        <a-form-item
          hidden
          label="id"
        >
          <a-input :min="1" style="width: 100%" v-decorator="['id']" />
        </a-form-item>
        <a-form-item
          label="菜单名称"
        >
          <a-input
            placeholder="菜单名称"
            v-decorator="[
              'menuName',
              {rules: [{ required: true, message: '请输入菜单名称' }]}
            ]"
          ></a-input>
        </a-form-item>

        <a-form-item
          label="菜单Code"
        >
          <a-input placeholder="菜单Code" style="width: 100%" v-decorator="['menuCode', {rules: [{ required: true, message: '请输入菜单Code' }]}]" />
        </a-form-item>

        <a-form-item
          label="父级"
        >
          <a-tree-select
            style="width: 100%"
            :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }"
            :treeData="menuTreeData"
            :replaceFields="{children:'children', title:'menuName', key:'id', value: 'id' }"
            placeholder="请选择父级"
            allow-clear
            tree-default-expand-all
            v-decorator="['parentId', {rules: [{ required: true, message: '请选择父级' }], initialValue: parentId}]"
          >
          </a-tree-select>
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
</template>

<script>
import { systemMenuUpdate, systemMenuTree, systemMenuDetail } from '@/api/clever-framework'
import { TreeSelect as ATreeSelect } from 'ant-design-vue'
import pick from 'lodash.pick'

// 表单字段
const fields = ['id', 'menuName', 'menuCode', 'parentId']

export default {
  name: 'SystemMenuCreate',
  components: {
    ATreeSelect
  },
  props: {
    record: {
      type: [Object],
      required: false,
      default: null
    }
  },
  data () {
    return {
      labelCol: {
        xs: { span: 24 },
        sm: { span: 5 }
      },
      wrapperCol: {
        xs: { span: 24 },
        sm: { span: 12 }
      },
      loading: false,
      visible: false,
      menuTreeData: [],
      parentId: '0',
      form: this.$form.createForm(this)
    }
  },
  mounted () {
  },
  methods: {
    show () {
      this.visible = true
      this.loadMenuTree()
      debugger
      if (this.record && this.record.id) {
        this.loadDetailInfo(this.record.id)
      }
    },
    handleOk () {
      this.handleSubmit()
    },
    handleCancel () {
      this.loading = false
      this.visible = false
      // 重置表单数据
      this.form.resetFields()
      this.$emit('cancel')
    },
    loadMenuTree () {
      systemMenuTree().then(response => {
        this.menuTreeData = response.content
      })
    },
    handleSubmit () {
      const { form: { validateFields } } = this
      validateFields((err, values) => {
        if (!err) {
          this.loading = true
          // 新增
          systemMenuUpdate(values).then(res => {
            this.visible = false
            // 重置表单数据
            this.form.resetFields()
            this.$message.info('修改成功')
            this.$emit('ok')
          }).catch(errors => {
            this.$message.error(errors.message || '修改失败')
          }).finally(() => {
            this.loading = false
          })
        }
      })
    },
    loadDetailInfo (id) {
      // ajax
      console.log(`将加载 ${id} 信息到表单`)
      this.loading = true
      systemMenuDetail(id).then((response) => {
        const { content } = response
        const formData = pick(content, fields)
        console.log('formData', formData)
        this.form.setFieldsValue(formData)
      }).finally(() => {
        this.loading = false
      })
    }
  }
}
</script>
